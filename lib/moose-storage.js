...
function validateMoose(moose, fixup2017Moose = false) {
    ...
    else if (isNaN((new Date(moose.created)).getTime())) {
        moose.created = new Date();
    }
    else {
    ...
    if (moose.name === 'random' || moose.name === 'latest' || moose.name === 'oldest') {
        if (!fixup2017Moose) {
            return 'Moose.name cannot be the reserved words "latest", "oldest" or "random".';
        }
        else {
...
/**
   Save a new moose or reject it as invalid.
   @param {Moose} moose
   @return {Promise<string>} status message.
   @throws {MooseStorageError} with a message why the moose is not valid.
*/
function newMoose(moose) {
    // Force the user incoming moose to be *new* and to use the server's time.
    moose.created = null;
...
/**
 * Get a single moose.
 * @param {(string|'random'|'latest'|'oldest')} q - name of moose.
 ...
 * @return {Promise<Buffer>} a PNG of the moose.
 * @throws {MooseStorageError}
 */
async function getImage(name) {
    ...
    if (img) {
        ...
        const moose_1 = await getMoose(name);
        const png = await pngOutput(moose_1);
        // @ts-ignore
