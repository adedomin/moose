...
function mooseHandler(resolve, reject, moose, name) {
    if (moose === undefined) {
        reject(new MooseStorageError(`${MooseStorageError.messageEnum.NO_SUCH_MOOSE} ${name}`));
    }
    else {
        resolve(moose);
    }
