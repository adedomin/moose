    ...
    register() {
        ...
        this.irc.matchMessage(/^[.!]?moose(?:me)?\b/, (event) => {
            ...
            if (arg === '--help' || arg === '-h') {
                ...
                needle(
                    'get',
                    `${mooseUrl}/gallery/newest?p=0&q=${query}`,
                ).then(res => {
                ...
                needle(
                    ...
                    if (res.statusCode !== 200) throw res.body;
                    const relativeDate = reladate(new Date(res.body.created));
                    const outputBody = ircLine(res.body);
                    this.outputMoose(
                        event,
                        outputBody,
                        '\x02' + res.body.name + '\x02' +
                        ` - ${mooseUrl}/img/${encodeURIComponent(res.body.name)}` +
                        ' - ' + relativeDate,
                    );
                }).catch(err => {
                    if (err.msg) {
