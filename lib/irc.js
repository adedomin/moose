...
const mooseImageSpam = antispam(config.irc.moose_image_spam || 5000);
...
module.exports = (moosedb) => {
    ...
    irc.matchMessage(/^[.!](bots|help)/, event => {
        if (mooseImageSpam(event.target)) return;
        event.reply(
    ...
    irc.matchMessage(/^[.!]?mooseimg\b/, function(event) {
        ...
        if (!query) {
            event.reply(
                `Get image of any moose with: ${config.irc.moose_url || 'https://moose.ghetty.space'}/img/<moose-name>`,
            );
        }
