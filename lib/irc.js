...
function outputMoose(event, rawOutput, err, moose, query) {
    if (err) {
        mooseLock = false;
        if (err.toString() === 'no such moose') {
            logger.debug('IRC MOOSE OUTPUT', { err: `no such moose: ${query}` });
        }
        else {
            logger.error('IRC MOOSE OUTPUT ERROR', { err: `requested moose: ${query}, ${err}` });
        }
...
module.exports = (moosedb) => {
    ...
    irc.matchMessage(/^[.!]?moose(?:me)?\b/, function(event) {
        ...
        const boundOutputMoose = (err, moose) => {
            outputMoose(event, ircRawOut, err, moose, query);
        };
