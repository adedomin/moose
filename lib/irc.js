...
const helpSpam = antispam(config.irc.help_spam || 10000);
const mooseSearchSpam = antispam(config.irc.moose_search_spam || 10000);
const mooseHelpSpam = antispam(config.irc.moose_help_spam || 10000);
const moosePleaseWaitSpam = antispam(config.irc.moose_please_wait_spam || 10000);
let mooseLock = false;
    ...
    irc.matchMessage(/^[.!]?moose(?:me)?\b/, function(event) {
        ...
        if (arg == '--latest' ||
            ...
            moosedb.getMoose(query, (err, moose) => {
                ...
                output(moose, event, irc.raw.bind(irc), false, () => mooseLock = false);
            });
        }
    });
