...
function ircColor(fore, back, str) {
    return '\x03' + parseInt(fore, 16).toString().padStart(2, '0')
        + (back !== -1 ? ',' + parseInt(back, 16).toString().padStart(2, '0') : '')
        + str + '\x03';
}

function mooseLine(image, shadeImg) {
    let retLine = '';
    for (let i = 0; i < image.length; ++i) {
        ...
        for (let j = 0; j < line.length; ++j) {
            ...
            if (char === 't') {
                ...
                retLine += ircColor(char, ...mooseShade[shade]);
            }
        }
        retLine += '\n';
    }
