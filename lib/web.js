...
function onErr(res, err) {
    ...
    if (!(err instanceof MooseStorageError) && err instanceof Error) {
        ...
        log.error(`WEB UNKNOWN ERR ${err.message}`, { err });
        code = 500;
        userMessage = 'Unknown Error Occured.';
    }
    else if (!(err instanceof Error)) {
        log.error(`WEB UNKNOWN ERR TYPE ${err}`);
        code = 500;
        userMessage = 'Unknown Error Occured.';
    }
    else if (userMessage.indexOf(MooseStorageError.messageEnum.NO_SUCH_MOOSE) === 0) {
        code = 404;
    }
    else {
        // these errors are most likely tame, but debuggable if they are not.
        // e.g. No such moose, no moose matching search, etc.
        log.debug(`WEB DEBUG ${err.message}`, { err });
    }
