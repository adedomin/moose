...
function mooseTrimTransparent(orlines) {
    // copy arr
    let lines = orlines.slice();
    // top down
    const tline = /^t*$/;

    let nonblank_start = 0;

    // top down
    nonblank_start = lines.findIndex(line => !tline.test(line));
    if (nonblank_start !== -1) {
        lines = lines.slice(nonblank_start);
    }
    ...
    nonblank_start =
        lines
            .slice()
            .reverse()
            .findIndex(line => !tline.test(line));
    if (nonblank_start !== -1) {
        lines = lines.slice(0, -nonblank_start);
    }
...
// NOTE: raw is to circumvent broken whitespace output in v4.9.0 of irc-framework
